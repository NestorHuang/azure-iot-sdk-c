...
static pfIoTHubTransport_SetTransportCallbacks      IotHubTransportMqtt_WS_SetTransportCallbacks;
...
TEST_SUITE_INITIALIZE(suite_init)
{
    ...
    IotHubTransportMqtt_WS_SetTransportCallbacks = ((TRANSPORT_PROVIDER*)MQTT_WebSocket_Protocol())->IoTHubTransport_SetTransportCallbacks;
}
...
TEST_FUNCTION(IoTHubTransportMqtt_WS_SetTransportCallbacks_success)
{
    // arrange
    IOTHUBTRANSPORT_CONFIG config = { 0 };
    SetupIothubTransportConfig(&config, TEST_DEVICE_ID, TEST_DEVICE_KEY, TEST_IOTHUB_NAME, TEST_IOTHUB_SUFFIX, TEST_PROTOCOL_GATEWAY_HOSTNAME);
    ...
    STRICT_EXPECTED_CALL(IoTHubTransport_MQTT_SetTransportCallbacks(IGNORED_PTR_ARG, IGNORED_PTR_ARG, IGNORED_PTR_ARG));

    // act
    int result = IotHubTransportMqtt_WS_SetTransportCallbacks(handle, transport_cb_info, NULL);
