...
TEST_FUNCTION(IoTHubTransportAmqp_SetTransportCallbacks_success)
{
    // arrange
    TRANSPORT_PROVIDER* provider = (TRANSPORT_PROVIDER*)AMQP_Protocol_over_WebSocketsTls();
    umock_c_reset_all_calls();

    STRICT_EXPECTED_CALL(IoTHubTransport_AMQP_SetTransportCallbacks(TEST_TRANSPORT_LL_HANDLE, g_transport_cb_info, g_transport_cb_ctx));

    // act
    provider->IoTHubTransport_SetTransportCallbacks(TEST_TRANSPORT_LL_HANDLE, g_transport_cb_info, g_transport_cb_ctx);
