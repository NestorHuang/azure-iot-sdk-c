...
static pfIoTHubTransport_SetTransportCallbacks      IoTHubTransportMqtt_SetTransportCallbacks;
...
TEST_SUITE_INITIALIZE(suite_init)
{
    ...
    IoTHubTransportMqtt_SetTransportCallbacks = ((TRANSPORT_PROVIDER*)MQTT_Protocol())->IoTHubTransport_SetTransportCallbacks;
}
...
TEST_FUNCTION(IoTHubTransportMqtt_SetTransportCallbacks_success)
{
    // arrange
    IOTHUBTRANSPORT_CONFIG config = { 0 };
    SetupIothubTransportConfig(&config, TEST_DEVICE_ID, TEST_DEVICE_KEY, TEST_IOTHUB_NAME, TEST_IOTHUB_SUFFIX, TEST_PROTOCOL_GATEWAY_HOSTNAME);
    ...
    STRICT_EXPECTED_CALL(IoTHubTransport_MQTT_SetTransportCallbacks(IGNORED_PTR_ARG, g_transport_cb_info, g_transport_cb_ctx));

    // act
    int result = IoTHubTransportMqtt_SetTransportCallbacks(handle, g_transport_cb_info, g_transport_cb_ctx);
